...
**Q01.** 다음은 UML (   ) 다이어그램이다. 아래 내용을 보고 다이어그램의 명칭을 작성하시오.
...
**Q02.** 다음은 소프트웨어 테스트 기법 중 하나에 대한 설명이다. 보기에서 골라 쓰시오.
...
각각의 조건식이 True와 False 두 가지 경우를 모두 한 번 이상 만족하도록 테스트 케이스를 설계해야 한다.
...
> \[보기\]
> ㄱ. 경로(Path) ㄴ. 결정(Decision) ㄷ. 조건/결정(Condition/Decision) ㄹ. 변경 조건/결정(MC/DC) ㅁ. 다중 조건(Multiple Condition) ㅂ. 문장(Statement) ㅅ. 분기(Branch) ㅇ. 조건(Condition) ㅈ. 루프(Loop)

---

**Q03.** 다음은 유닉스(Unix) 또는 리눅스(Linux) 환경에서 자주 사용하는 기본 명령어에 대한 설명이다. 각 설명에 맞는 명령어를 보기에서 골라 쓰시오.
...
**Q04.** 다음은 오류 검출 방식을 설명하는 내용이다. 빈칸 ①~⑤에 들어갈 알맞은 용어를 \<보기\>에서 고르시오.
...
**Q05.** 다음은 C코드에 대한 문제이다. 아래 코드를 확인하여 알맞는 출력값을 작성하시오.
...
**Q06.** 다음은 C코드에 대한 문제이다. 아래 코드를 확인하여 알맞는 출력값을 작성하시오.
...
**Q07.** 다음은 C코드에 대한 문제이다. 아래 코드를 확인하여 알맞는 출력값을 작성하시오.
...
**Q08.** 아래 코드는 Machine이라는 인터페이스를 정의하고 WashingMachine 클래스에서 해당 인터페이스를 사용하고자 한다. 빈칸에 들어갈 올바른 키워드를 작성하시오.
...
**Q09.** 다음은 파이썬에 대한 문제이다. 아래 코드를 확인하여 출력값의 알맞는 값을 작성하시오.
...
**Q10.** 다음은 테스트케이스를 실행한 화면이다. 이에 대한 알맞는 결과값을 작성하시오.
...
**Q12.** 다음은 Java의 상속과 생성자 호출에 관한 코드이다. 밑줄에 알맞은 단어를 작성하시오.
...
**Q13.** 다음은 인증 및 자원 접근 방식에 대한 설명이다. 알맞은 단어를 작성하시오.
...
**Q14.** 다음 아래의 테이블을 확인하여 R÷S의 결과를 테이블 형태로 기재하시오.
...
**Q15.** 다음은 C코드에 대한 문제이다. 아래 코드를 확인하여 알맞는 출력값을 작성하시오.
...
**Q16.** 관계형 데이터베이스 개념에 대한 설명이다. 빈칸에 들어갈 용어를 \<보기\>에서 골라 순서대로 쓰시오.
...
**Q17.** 다음은 Java에 대한 코드이다. 알맞는 출력값을 작성하시오.
...
**Q18.** 다음은 정보보안에서 사용하는 접근통제(Access Control) 방식에 대한 설명이다. 설명에 해당하는 접근통제 모델을 \<보기\>에서 골라 빈칸에 작성하시오.
...
**Q19.** 다음은 테스트케이스의 구성요소에 대한 설명이다. 괄호 ( ) 안에 들어갈 알맞는 보기를 고르시오.
...
**Q20.** 다음은 SQL에 관한 문제이다. 아래 A테이블을 참고하여 쿼리의 결과를 작성하시오.
...
|------|------|
| Q01 | 패키지(Package) |
| Q02 | ㅇ. 조건(Condition) 커버리지 |
| Q03 | ①pwd, ②ls, ③cd, ④cp |
| Q04 | ①Hamming, ②FEC, ③BEC, ④Parity, ⑤CRC |
| Q05 | C |
| Q06 | E |
| Q07 | 187 |
| Q08 | implements |
| Q09 | ①15, ②5, ③10, ④3, ⑤18, ⑥5, ⑦9, ⑧2 |
| Q10 | DS-45S-21: 로그인 성공, DS-45S-25: 로그인 실패 |
| Q11 | OTP (One-Time Password) |
| Q12 | super |
| Q13 | OAuth |
| Q14 | A: a1 |
| Q15 | 0 |
| Q16 | ㄱ: 튜플(Tuple), ㄴ: 인스턴스(Instance), ㄷ: 카디널리티(Cardinality) |
| Q17 | AB |
| Q18 | ㄱ: MAC, ㄴ: RBAC, ㄷ: DAC |
| Q19 | ㄱ(테스트 조건), ㄹ(테스트 데이터), ㅁ(예상 결과) |
| Q20 | 4 |
